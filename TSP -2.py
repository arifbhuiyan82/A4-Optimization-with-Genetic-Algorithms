...
def getCity(problem_file):
    ...
    cities = [(str(node), float(coords[0]), float(coords[1])) for node, coords in problem.node_coords.items()]
    print("Cities read from file:", cities)
    return cities
...
def get_user_input():
    ...
    return POPULATION_SIZE, TOURNAMENT_SELECTION_SIZE, MUTATION_RATE, CROSSOVER_RATE, TARGET
...
def drawMap(city, answer):
    if not answer or len(answer) < 2 or not answer[1]:
...
def main():
    POPULATION_SIZE, TOURNAMENT_SELECTION_SIZE, MUTATION_RATE, CROSSOVER_RATE, TARGET = get_user_input()

    cities = getCity("Qatar194.tsp")  
